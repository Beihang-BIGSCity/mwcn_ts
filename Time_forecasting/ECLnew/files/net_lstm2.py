...
class netlstm2(nn.Module):#三层小波分解
    def __init__(self, length_input,time_step,output_size=96,mode='dot',factor=5, n_heads=8, d_layers=2,
                dropout=0.0, model='cnn',d_prob=0.5,hidden_size=6):
        super(netlstm2, self).__init__()   # 继承__init__功能
...
#        self.features2 = Conv_multiwaveH(is_training=True,len_input=self.len_input,sim_reg=0,activation=torch.sigmoid)
        ...
        self.embed1 = nn.LSTM(input_size=2, hidden_size=self.hidden_size, batch_first=True,bidirectional=True)#本input 任意的送进的（非，是一个模型） 需要送进的 第一个为本int(time_step) [L,N,H_in] when batch_first=true [N,L,H_in] when batch_first=true
        self.embed2 = nn.LSTM(input_size=2, hidden_size=self.hidden_size, batch_first=True,bidirectional=True)#int(time_step/2)
        self.embed3 = nn.LSTM(input_size=2, hidden_size=self.hidden_size, batch_first=True,bidirectional=True)#-2 int(time_step/4)
...
#        self.output = nn.Linear(in_features=32*(int(self.len_input/8))*2, out_features=self.num_outputs)
        ...
        self.fc1 = nn.Linear(in_features=2*hidden_size,out_features=1)
        self.output = nn.Linear(in_features=int((self.len_input)/2+(self.len_input)/4+(self.len_input)/8), out_features=self.output_size)
...
#        self.embedding = DataEmbedding(enc_in, d_model, embed, freq, dropout)
    def forward(self, x,mode='dot'):
        ...
        if self.diff !=0:
            if x.shape[0]==16:
                btr = np.zeros((16,self.diff))#去中间1品
                ad1 = torch.from_numpy(btr).to(device=0)
                ##print("x,ad1",x.shape,ad1.shape)
                #x = x.unsqueeze(1)
                x = torch.cat((x,ad1),1)
            elif x.shape[0]!=16:
                btr = np.zeros((x.shape[0],self.diff))
                ad1 = torch.from_numpy(btr).to(device=0)
                ##print("x,ad1",x.shape,ad1.shape)
                #x = x.unsqueeze(1)
                x = torch.cat((x,ad1),1)
...
#后段代码本身所在位置
        ...
        if self.model == 'res':
            ...
            low1 = low1.to(torch.float32)
            low1=low1.unsqueeze(2)
            #print(type(all_out1))
            high1 = high1.to(torch.float32)
            high1=high1.unsqueeze(2)
            ...
            low2=low2.unsqueeze(2)
            high2 = high2.to(torch.float32)
            high2 = high2.unsqueeze(2)
            ...
            low3=low3.unsqueeze(2)
            high3 = high3.to(torch.float32)
            high3 = high3.unsqueeze(2)#周代码写的复杂 用它中的预测框架 那不用他的预测框架 自混？
...
#            all_out3 = self.maxp(all_out3)
            ##print("all_out",all_out1.shape,all_out2.shape,all_out3.shape)
            z = torch.cat([all_out1,all_out2,all_out3],dim=1)#2
            ##print("z shape:",z.shape)
            z=self.fc1(z)
            z=z.squeeze(2)#有地方等 有地方不等品 squeeze(0)品
