...
#        super(Conv_waveL, self).__init__()
    def __init__(self,  is_training, len_input=96, sim_reg=0, activation=None):
        ...
        print("self.len_input",len_input)
        L1,H1 = mdwt_matrix(self.len_input,'sa4')
    ...
    def forward(self,input):
        p,q = pmatrix(self.len_input,'sa4ap')
        p1 = torch.tensor(p,dtype=torch.float32).to(device=0)
...
#        hp_out = torch.matmul(self.hp_weight,hp_out)
        lp_out = torch.mm(self.lp_weight,p1).t()
        input = input.float()
        ##print("input lp_out",input.shape,lp_out.shape)
        lp_out = torch.matmul(input,lp_out)
        hp_out = torch.mm(self.hp_weight,p1).t()
...
class Conv_multiwaveOld(nn.Module):
#    def __init__(self,  is_training, l1_value, weight_decay,len_input=96, sim_reg=0, activation=None):
#        super(Conv_waveL, self).__init__()
    def __init__(self,  is_training, len_input=96, sim_reg=0, activation=None):
        super(Conv_multiwaveH, self).__init__()
        self.len_input = len_input
        L1,H1 = mdwt_matrix(self.len_input,'sa4')
        lp_weight0 = torch.tensor(L1,dtype=torch.float32, requires_grad=True)
        self.lp_weight = torch.nn.Parameter(lp_weight0,requires_grad = True)
        hp_weight0 = torch.tensor(H1,dtype=torch.float32, requires_grad=True)
        self.hp_weight = torch.nn.Parameter(hp_weight0,requires_grad = True)
        self.activation = activation
    def forward(self,input):
        p,q = pmatrix(self.len_input,'sa4ap')
...
#        super(Conv_waveL, self).__init__()
    def __init__(self,  is_training, len_inputi=96, sim_reg=0, activation=None):
        ...
        print(len_inputi)
        Li,Hi = mdwt_matrix(self.len_input,'sa4')
...
#        hp_outi = hp_outi + biases_hpi
        if not self.activation==None:
            hp_outi = self.activation(hp_outi)
            lp_outi = self.activation(lp_outi)
