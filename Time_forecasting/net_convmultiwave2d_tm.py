...
import torch.nn.functional as F
from SENet import *
from models.TimeMixer.TimeMixer import Model
...
class conv2d_multiwave_tm(nn.Module):
    def __init__(self, configs, length_input, time_step, output_size=96, mode='dot', factor=5, d_model=512, n_heads=8, d_layers=2,
        ...
        super(conv2d_multiwave_tm, self).__init__()
        ...
        configs.num_nodes = 4
        self.encoder1 = Model(configs, self.len_input // 4, self.len_input // 4, hidden_size)
        self.encoder2 = Model(configs, self.len_input // 4, self.len_input // 4, hidden_size)
        self.encoder3 = Model(configs, self.len_input // 4, self.len_input // 4, hidden_size)
