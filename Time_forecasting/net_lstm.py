...
#        self.features2 = Conv_multiwaveH(is_training=True,len_input=self.len_input,sim_reg=0,activation=torch.sigmoid)
        ...
        self.embed1 = nn.LSTM(input_size=int(time_step), hidden_size=self.hidden_size, batch_first=True,bidirectional=True)#本input 任意的送进的（非，是一个模型） 需要送进的 第一个为本 
        self.embed2 = nn.LSTM(input_size=int(time_step/2), hidden_size=self.hidden_size, batch_first=True,bidirectional=True)#int(time_step/2)
        self.embed3 = nn.LSTM(input_size=int(time_step/4), hidden_size=self.hidden_size, batch_first=True,bidirectional=True)#-2
