...
parser.add_argument('--log_dir', default='ECL96', help='Log dir [default: log]')
...
#        correct += predicted.eq(targets).sum().item()
        ...
        progress_bar(batch_idx, len(trainloader), 'MSELoss: %.3f |Total:%d'
...
#            print(targets)
            total += targets.size(0)
#            correct += predicted.eq(targets).sum().item()
            progress_bar(batch_idx, len(testloader), 'MSELoss: %.3f |(total:%d)'% (test_loss/(batch_idx+1), total))
...
for epoch in range(start_epoch, start_epoch+20):
    ...
    targets = targets.view(-1).data.numpy()
