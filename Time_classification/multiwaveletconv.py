# -*- coding: utf-8 -*-
"""
Created on Fri Apr  1 11:43:39 2022

@author: 13621 multiwavelet conv
"""
...
#        super(Conv_waveL, self).__init__()
    def __init__(self,  is_training, len_input=96, sim_reg=0, activation=None):
        super(Conv_mwavepre, self).__init__()
    ...
    def forward(self,input):
        ...
        L01 = self.convpre1(input)
        L02 = self.convpre2(input)
        lpre0 = torch.cat([L01,L02],dim=2)
        #print(lpre0.shape[2])
        for i in range(lpre0.shape[2]):
            if i%2==0:
                lpre0[:,0,i]=L01[:,0,int(i/2)]
            elif i%2==1:
                lpre0[:,0,i]=L02[:,0,int((i-1)/2)]
        return lpre0
...
class Conv_mwavedec(nn.Module):
#    def __init__(self,  is_training, l1_value, weight_decay,len_input=96, sim_reg=0, activation=None):
#        super(Conv_waveL, self).__init__()
    def __init__(self,  is_training, len_inputi=96, sim_reg=0, activation=None):
        super(Conv_mwavedec, self).__init__()
        self.len_input = len_inputi
        kl1,kl2,kh1,kh2 = mdwt_kernel(self.len_input,'ghm')
        self.convdecl1 = nn.Conv1d(in_channels=1, out_channels=1, kernel_size=8,stride=4,padding=0,dilation=1,bias=False)
        kl_1=torch.Tensor(kl1)
        self.convdecl1.weight=torch.nn.Parameter(kl_1) 
        self.convdecl2 = nn.Conv1d(in_channels=1, out_channels=1, kernel_size=8,stride=4,padding=0,dilation=1,bias=False)
        kl_2=torch.Tensor(kl2)
        self.convdecl2.weight=torch.nn.Parameter(kl_2)
        self.convdech1 = nn.Conv1d(in_channels=1, out_channels=1, kernel_size=8,stride=4,padding=0,dilation=1,bias=False)
        kh_1=torch.Tensor(kh1)
        self.convdech1.weight=torch.nn.Parameter(kh_1)
        self.convdech2 = nn.Conv1d(in_channels=1, out_channels=1, kernel_size=8,stride=4,padding=0,dilation=1,bias=False)
        kh_2=torch.Tensor(kh2)
        self.convdech2.weight=torch.nn.Parameter(kh_2)
    def forward(self,input):
        in1 = input[:,:,0].unsqueeze(2)
        in2 = input[:,:,1].unsqueeze(2)
        in3 = input[:,:,2].unsqueeze(2)
        in4 = input[:,:,3].unsqueeze(2)
        input = torch.cat([input,in1,in2,in3,in4],dim=2)
        ...
        for i in range(lp_out.shape[2]):
            if i%2==0:
                lp_out[:,0,i]=lp_out1[:,0,int(i/2)]
            elif i%2==1:
                lp_out[:,0,i]=lp_out2[:,0,int((i-1)/2)]
        hp_out = torch.cat([hp_out1,hp_out2],dim=2)
        for j in range(hp_out.shape[2]):
            if j%2==0:
                hp_out[:,0,j]=hp_out1[:,0,int(j/2)]
            elif j%2==1:
                hp_out[:,0,j]=hp_out2[:,0,int((j-1)/2)]
