...
class multiCNNatt1(nn.Module):#三层小波分解
    def __init__(self,length_input = 96, number_ouputs=4,mode='dot',factor=5, d_model=512, n_heads=8, d_layers=2,
        ...
        super(multiCNNatt1, self).__init__()   # 继承__init__功能
        if length_input % 8 != 0:
            aaa = length_input%8
...
#        self.resnet = ResNet1D(in_channels=1, base_filters=32,kernel_size=kernel_size,stride=stride,groups=4,n_block=n_block,n_classes=number_ouputs,downsample_gap=downsample_gap,increasefilter_gap=increasefilter_gap,use_do=True)# 64 for ResNet1D, 352 for ResNeXt1D
        ...
        self.vgg1d5 =  VGG1d('VGG13',num_class=self.num_outputs)
        self.vgg1d6 =  VGG1d('VGG16',num_class=self.num_outputs)
        self.vgg1d7 =  VGG1d('VGGnew',num_class=self.num_outputs)
#        self.output = nn.Linear(in_features=32*(int(self.len_input/8))*2, out_features=self.num_outputs)
        self.fc = nn.Linear(in_features=3*self.num_outputs,out_features=self.num_outputs)#14976
        self.output = nn.Linear(in_features=self.num_outputs*3, out_features=self.num_outputs)
...
#        self.embedding = DataEmbedding(enc_in, d_model, embed, freq, dropout)
    def forward(self, x,mode='dot'):
        ...
        ##print("high1 shape",high1.shape)
        low2,high2 = self.features3(low1)
        ##print("low2 shape",low2.shape)
        ##print("high2 shape",high2.shape)
#print(low2.shape)
        low2 = low2.to('cuda:0')
...
#            print(low1.shape)
            ...
            all_out2 = self.re(all_out2)
            #all_out2 = self.conv2d21(all_out2)           # [batch, 32,48]
            #all_out2 = self.re(all_out2)
            all_out2 = self.maxp(all_out2)
            all_out2 = self.conv2d3(all_out2)
            all_out2 = self.re(all_out2)
            #all_out2 = self.conv2d31(all_out2)
            #all_out2 = self.re(all_out2)
            all_out2 = self.maxp(all_out2)
...
#            low3 = low3.to(torch.float32)
            ...
            all_out3 = self.re(all_out3)
            all_out3 = self.conv2d21(all_out3) # [batch, 32,48]
            all_out3 = self.re(all_out3)
            all_out3 = self.maxp(all_out3)
            all_out3 = self.conv2d3(all_out3)
            all_out3 = self.re(all_out3)
            all_out3 = self.conv2d31(all_out3)
            all_out3 = self.re(all_out3)
            all_out3 = self.maxp(all_out3)
            all_out3 = all_out3.contiguous().view(x.size(0), -1)
            #print(all_out3.shape)
            z=torch.cat([all_out1,all_out2],dim=1)#,all_out3
            #out = z.view(z.size(0), -1)
            #print(z.shape)
            output = self.fc(z)     # 输出[16,2]
            return output
            ...
            high1=high1.unsqueeze(1)
            all_out1 = torch.cat([low1, high1],dim=1).to('cuda:0')
            all_out1 = self.vgg1d4(all_out1)
            low2 = low2.to(torch.float32)
            low2=low2.unsqueeze(1)
            high2 = high2.to(torch.float32)
            high2 = high2.unsqueeze(1)
            all_out2 = torch.cat([low2, high2],dim=1).to('cuda:0')
            all_out2 = self.vgg1d3(all_out2)#原为2
...
#            print(all_out3.shape)
            ...
            #print(f"shape of z after reshape:{output.shape}")
            #output = nn.Linear(in_features=15, out_features=self.num_outputs)
