...
class conv2d_multiwave_v2_att33(nn.Module):#三层小波分解
    def __init__(self,length_input = 96, number_ouputs=4,mode='dot',factor=5, d_model=512, n_heads=8, d_layers=2,
        ...
        super(conv2d_multiwave_v2_att33, self).__init__()   # 继承__init__功能 pytorch初始化参数
        if length_input % 8 != 0:
            aaa = length_input%8
    ...
    def channel_att(self,u):
        ...
        z = torch.sum(u1,dim=3)#[16,12,1,1]
        ...
        #s =s.squeeze(2)
        s= s.unsqueeze(3)
        u_widetilde = torch.mul(u,s)#[16,12,1,num]*[16,12,1,1]
        #for c in range(0,12):
    ...
    def forward(self, x,mode='dot'):
        ...
        if self.model=='vgg':    
            ...
            u = torch.stack([low11,low12,high11,high12,low21,low22,high21,high22,low31,low32,high31,high32],dim=1)#维度yuchannel的关系#[16,12,1,88] #原来是dim=2
            print(u.shape)
            ##u = u.squeeze(1)
            ...
            low11 = u1[:,0,:]
            low12 = u1[:,1,:]
            low1 = torch.cat([low11.unsqueeze(1),low12.unsqueeze(1)],dim=2)
            high1 = torch.cat([u1[:,2,:].unsqueeze(1),u1[:,3,:].unsqueeze(1)],dim=2)
            low2 = torch.cat([u1[:,4,:].unsqueeze(1),u1[:,5,:].unsqueeze(1)],dim=2)
            high2 = torch.cat([u1[:,6,:].unsqueeze(1),u1[:,7,:].unsqueeze(1)],dim=2)
            low3 = torch.cat([u1[:,8,:].unsqueeze(1),u1[:,9,:].unsqueeze(1)],dim=2)
            high3 = torch.cat([u1[:,10,:].unsqueeze(1),u1[:,11,:].unsqueeze(1)],dim=2)
